...
Различия между веб-сервером и веб-сервисом
- Веб-сервер используется для хранения и доставки статических файлов сайта, в то время как веб-сервис используется для обмена данными между различными системами и приложениями.
- Веб-сервер работает на основе http протокола, а веб-сервис использует различные протоколы, такие как SOAP и REST.
- Веб-сервер обычно не имеет встроенной логики, а веб-сервис может иметь определенную логику, связанную с обработкой и передачей данных.
Веб-сервер — это компьютер, который хранит файлы сайта (HTML-документы, CSS-стили, JavaScript-файлы, картинки и другие) и доставляет их на устройство конечного пользователя (веб-браузер и т.д.), 
используя HTTP-протокол. В случае обращения к какой-либо странице сайта, веб-сервер отправляет запрошенные данные пользователю.
Веб-сервис — это технология взаимодействия между системами и приложениями. Веб-сервисы работают на основе набора протоколов и стандартов с открытым исходным кодом. 
Например, REST подход подразумевает передачу представления ресурсов, таких как HTML, JSP или XML.
...
Server. Включает в себя информацию о сервере, который обрабатывал команду
Set-Cookie. Включает в себя куки, необходимые для идентификации клиента
Public. Перечисляет те запросы, которые может использовать любой.
...
Тёплый клиент — это клиент, у которого есть сформированный интерес к продукту компании, он знаком с брендом и, возможно, подписан на соцсети и рассылки.
#### Что такое холодный клиент?
Холодные клиенты – это аудитория, которая является целевой, но у нее нет осознания, что ей нужен именно ваш продукт. 
У этих людей есть слабая потребность решить проблему, они просто знакомятся с информацией и пока не собираются ничего покупать.
#### Что такое кеш?
...
Например необходимо проанализировать инцидент случившийся на продакшене и у нас нет доступа к этому окружению по известным всем причинам. 
Другая ситуация, если сервис работает на операционной системе Windows, а доступ нужен для трех и более сотрудников одновременно.
#### Что такое DNS?
...
PS: точно не понимаю в чем разница, кроме того, что URI это общее название. URN имеет постоянное название. URL это ссылка на страницу
...
Поиск сервера => Соединение и запрос => Ответ сервера => Отправка данных в браузер => Отображение страницы
#### Что такое HTTP?
